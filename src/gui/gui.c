...
int main_loop(void) {
    ...
    nextframe = SDL_GetTicks() + FRAME_TIME;
    ...
    while (!quit) {
        ...
        if (sockstate > 2) {
            // Decode as many ticks as we can and add their contents to the prefetch queue
            ...
            if (timediff < 0 || nexttick <= nextframe) {
